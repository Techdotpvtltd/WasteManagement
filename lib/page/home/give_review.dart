...
class GiveReview extends StatefulWidget {
  ...
  const GiveReview(
      {super.key, required this.isDrawer, required this.requestId});
...
class _GiveReviewState extends State<GiveReview> {
  ...
  void triggerRatingEvent(RequestBloc bloc) {
    bloc.add(RequestEventRateResponse(
        requestId: widget.requestId, review: controller.text, rating: rating));
  }
  ...
  Widget build(BuildContext context) {
    return BlocListener<RequestBloc, RequestState>(
      listener: (context, state) {
        if (state is RequestStateRating ||
            state is RequestStateRated ||
            state is RequestStateRatingFailure) {
          setState(() {
            isLoading = state.isLoading;
          });

          if (state is RequestStateRated) {
      ...
      child: Scaffold(
        ...
        floatingActionButton: gradientButton(
          ...
          ontap: () => triggerRatingEvent(context.read<RequestBloc>()),
